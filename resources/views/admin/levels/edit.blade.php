...
<div class="col-12">
    ...
    <hr>
    <form action="{{ route('admin.sublevels.update',['sublevel' => $sublevel['id']]) }}" method="post">
        @method('PUT')
        @include('admin.levels.levels_form')
        <hr>
        <button type="submit" class="btn btn-primary px-3 float-right">Save</button>
        <a href="{{ route('admin.exams.level.show', ['level_id' => $level['id']]) }}"
            class="btn btn-danger mx-3 float-right">Cancel</a>
    </form>
</div>
