        ...
        @if (count($heading['items']) > 0)
            ...
            @foreach ($heading['items'] as $nav_item)
                ...
                <li class="nav-item {{ is_active($nav_item['uri'] ?? '') }}">
                    <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapse{{ $nav_item['uri'] }}" aria-expanded="true"
                        aria-controls="collapse{{ $nav_item['uri'] }}">
                        <i class="{{ $nav_item['icon'] ?? '' }}"></i>
                        <span>{{ $nav_item['title'] ?? '' }}</span>
                    </a>
                    <div id="collapse{{ $nav_item['uri'] }}" class="collapse" aria-labelledby="headingPages" data-parent="#accordionSidebar">
